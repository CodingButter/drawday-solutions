 ...
 * with persistence to storage (chrome.storage.local or localStorage)
...
interface CollapsibleStateContextType {
  ...
  setSectionCollapsed: (
    section: keyof CollapsibleState,
    collapsed: boolean,
  ) => void;
}

const CollapsibleStateContext = createContext<
  CollapsibleStateContextType | undefined
>(undefined);
...
export function CollapsibleStateProvider({
  children,
}: {
  children: React.ReactNode;
}) {
  const [collapsedSections, setCollapsedSections] =
    useState<CollapsibleState>(DEFAULT_STATE);
    ...
    const loadState = async () => {
      try {
        if (typeof chrome !== "undefined" && chrome.storage) {
          const result = await chrome.storage.local.get("collapsibleSections");
          const stored = result.collapsibleSections;
          if (stored && typeof stored === "object") {
            setCollapsedSections({ ...DEFAULT_STATE, ...stored });
          }
        } else if (typeof window !== "undefined" && window.localStorage) {
          const stored = localStorage.getItem("collapsibleSections");
          if (stored) {
            const parsed = JSON.parse(stored);
            setCollapsedSections({ ...DEFAULT_STATE, ...parsed });
          }
        }
      } catch (error) {
...
export function useCollapsibleState() {
  ...
  if (!context) {
    throw new Error(
      "useCollapsibleState must be used within CollapsibleStateProvider",
    );
  }
  return context;
}
